...
2. `session_id` is the public-safe Python orchestrator runtime session key when emitted. Older/degraded Python paths retain `python-session`; mock remains `mock-session`.
...
| **Backend `session_id` (response)** | `ChatResponse.session_id` | Runtime session key emitted by Python when available; otherwise compatibility labels `python-session` or `mock-session`. It is still distinct from the UI-owned session key. |
...
| Topic | Issue |
| ----- | ----- |
| **Runtime `session_id`** | Clients may assume it is the UI conversation id; it is not. It identifies the Python runtime session when available. |
...
| `session_id` | Python runtime session id, or a compatibility label when unavailable — **see §2; do not treat as client session.** |
...
3. **Do not** replace the UI session with `response.session_id`; the frontend adapter prefers the echoed `client_session_id`. Use `session_id` for runtime correlation and optional **`conversation_id`** for server-backed conversation identity.
