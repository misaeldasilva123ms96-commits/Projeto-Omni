...
| Chat — client session correlation | Optional `client_session_id` in JSON body (UI `sessionId`) | `POST /chat` | ✅ AVAILABLE | `sendOmniMessage` forwards when `ChatClientContext.sessionId` set; Rust echoes after normalize (see `docs/backend/chat-session-contract.md`) | Pass through to Python stdin when orchestrator supports it | MEDIUM | LOW |
| Chat — `session_id` in response | Stable session id from runtime | `POST /chat` | ✅ AVAILABLE | Python emits a bounded `runtime_session_id`; Rust maps it to `session_id`. Legacy/degraded and mock paths keep explicit compatibility labels. | Keep UI continuity on `client_session_id`; treat `session_id` as runtime identity. | LOW | LOW |
...
| **Distinct session identities** | UI generates and preserves its own `client_session_id`; Rust returns the Python runtime session key in `session_id`. The frontend adapter prefers the echoed client id for UX continuity. `runtime_session_version` remains the Rust process epoch. |
