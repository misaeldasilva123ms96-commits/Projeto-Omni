...
def main() -> int:
    ...
    for record in records:
        ...
        if source == "synthetic_controlled":
            synthetic_count += 1
        if source in {"runtime_harvest", "runtime_feedback"}:
            runtime_count += 1
        ...
        if float(record.get("quality_score", 0.0) or 0.0) >= args.quality_threshold:
            above_threshold += 1
    readiness = "NOT_READY"
    if len(records) >= 120 and above_threshold >= 80:
        readiness = "READY_FOR_SMALL_LORA"
    if len(records) >= 500 and above_threshold >= 300 and approved_count >= 120:
        readiness = "READY_FOR_BROADER_LORA"
    payload = {
