...
// ---------------------------------------------------------------------------
// Phase 5 — Input Validation & Rate Limiting (Roadmap Oficial v2.1)
// ---------------------------------------------------------------------------

/// Read OMNI_MAX_MESSAGE_CHARS from env (default 8000). Alias: OMINI_MAX_MESSAGE_CHARS.
fn max_message_chars() -> usize {
    std::env::var("OMNI_MAX_MESSAGE_CHARS")
        .or_else(|_| std::env::var("OMINI_MAX_MESSAGE_CHARS"))
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .unwrap_or(8000)
}

/// Read OMNI_MAX_SESSION_ID_CHARS from env (default 128).
fn max_session_id_chars() -> usize {
    std::env::var("OMNI_MAX_SESSION_ID_CHARS")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .unwrap_or(128)
}

/// Strip or reject dangerous control characters from a message string.
/// Returns Err if the message contains non-printable ASCII control chars (except whitespace).
fn validate_message_chars(message: &str) -> Result<(), AppError> {
...
/// Validate a session_id string: alphanumeric, hyphens, underscores only.
...
/// Run all input validations for a chat message.
fn validate_chat_input(message: &str, session_id: Option<&str>) -> Result<(), AppError> {
    if message.is_empty() {
        return Err(AppError::InvalidRequest("message must not be empty".to_string()));
    }
    ...
    validate_message_chars(message)?;
    if let Some(sid) = session_id {
        if !sid.is_empty() && !validate_session_id(sid) {
            return Err(AppError::InvalidRequest(
                "client_session_id contains invalid characters or exceeds length limit".to_string(),
            ));
        }
    }
    Ok(())
...
async fn chat(
    ...
    let raw_session_id = payload.client_session_id.as_deref().unwrap_or("");
    validate_chat_input(&message, Some(raw_session_id))?;
