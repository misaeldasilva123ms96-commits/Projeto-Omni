...
_SECRET_PATTERNS = [
    # API keys — OpenAI project keys
    (re.compile(r"sk-proj-[a-zA-Z0-9_-]{20,}"), "[REDACTED_API_KEY]"),
    # API keys — generic sk- pattern
    ...
    (re.compile(r"\+?55\s?\(?\d{2}\)?\s?\d{4,5}[-.]?\d{4}"), "[REDACTED_PHONE]"),
    # CPF (Brazilian tax ID)
    (re.compile(r"\d{3}\.?\d{3}\.?\d{3}-?\d{2}"), "[REDACTED_CPF]"),
    # Generic secrets in key=value form
    (re.compile(r"(?i)(password|token|secret|api_key)\s*=\s*\S+"), r"\1=[REDACTED_SECRET]"),
    # Unix absolute paths
    (re.compile(r"/(?:home|root|tmp|var|usr|etc)/[a-zA-Z0-9_./-]+"), "[REDACTED_PATH]"),
    # Windows absolute paths
    (re.compile(r"[A-Za-z]:\\(?:Users|Program Files|Windows)[\\a-zA-Z0-9_\s.-]+"), "[REDACTED_PATH]"),
]
...
class LearningLogger:
    ...
    def _execution_success(
        ...
        # NOTE (Phase 9): MATCHER_SHORTCUT and SAFE_FALLBACK intentionally excluded.
        # Matcher and fallback responses must NOT be recorded as positive learning.

    @staticmethod
    def _is_positive_learning_candidate(
        runtime_mode: str,
        fallback_triggered: bool,
        ...
        Returns False for any execution that should NOT be used as positive training data.
        Matcher, fallback, tool failures, governance blocks, and provider failures
        must only produce diagnostic_event, negative_example, or failure_memory records.
        """
        if fallback_triggered:
            return False
        if runtime_mode in {"MATCHER_SHORTCUT", "SAFE_FALLBACK", "RULE_BASED_INTENT"}:
            return False
    ...
    def classify_memory_record(
        ...
        """
        Phase 9 — Return the appropriate memory classification for a learning record.
        """
        if runtime_mode in {"MATCHER_SHORTCUT", "RULE_BASED_INTENT"}:
            return "routing_eval_case"
        if runtime_mode == "SAFE_FALLBACK" or fallback_triggered:
            return "diagnostic_event"
        if not success or tool_status in {"failed", "blocked"}:
