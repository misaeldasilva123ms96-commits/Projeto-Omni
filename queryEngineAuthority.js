...
/**
 * Sanitize an error object for internal debug logging.
 * NEVER logs stack, cause, path, env, syscall, or raw payloads.
 * Only exposed when OMNI_DEBUG_INTERNAL_ERRORS=true AND NOT public-demo mode.
 */
function sanitizeErrorForInternalDebug(err) {
  ...
  return {
    name: String(err.name || 'Error').slice(0, 50),
...
function _isDebugInternalErrors() {
  return (
    String(process.env.OMNI_DEBUG_INTERNAL_ERRORS || '').trim() === 'true' &&
    String(process.env.OMNI_PUBLIC_DEMO_MODE || '').trim() !== 'true' &&
    String(process.env.OMINI_PUBLIC_DEMO_MODE || '').trim() !== 'true'
  );
}
...
function safeSpecialistCall(fn, args) {
  try {
    ...
    if (_isDebugInternalErrors()) {
      // eslint-disable-next-line no-console
      console.error('[specialist] fallback:', sanitizeErrorForInternalDebug(err));
    }
    return {
      invoked: true,
      degraded: true,
      specialist_id: fn.name || 'unknown',
      fallback: true,
