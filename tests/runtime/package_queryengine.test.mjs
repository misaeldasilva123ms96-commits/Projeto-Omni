...
const originalEngineAdoptionExists = fs.existsSync(engineAdoptionPath)
const originalEngineAdoptionContents = originalEngineAdoptionExists
  ? fs.readFileSync(engineAdoptionPath, 'utf8')
  : ''
...
try {
  ...
  if (originalEngineAdoptionExists) {
    fs.mkdirSync(path.dirname(engineAdoptionPath), { recursive: true })
    fs.writeFileSync(engineAdoptionPath, originalEngineAdoptionContents, 'utf8')
  } else {
    fs.rmSync(engineAdoptionPath, { force: true })
  }
}
