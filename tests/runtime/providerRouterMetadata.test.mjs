...
withProviderEnv({}, ({ DEFAULT_FALLBACK_CHAIN, FALLBACK_REASONS, getProviderRegistry, chooseProvider }) => {
  ...
  assert.deepEqual(Object.values(FALLBACK_REASONS).sort(), [
    'auto_routing_no_valid_candidate',
  ...
  assert.equal(openrouter.executable, true);
  assert.equal(openrouter.available, false);
  assert.equal(openrouter.reachable, null);
  assert.equal(openrouter.healthy, null);
  ...
  assert.equal(openai.executable, true);
  ...
  assert.equal(gemini.executable, true);
  ...
  assert.equal(ollama.executable, true);
  ...
  assert.equal(lmstudio.executable, true);
