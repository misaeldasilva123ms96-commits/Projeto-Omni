...
export function OmniRightInspector({ children, className = '', defaultOpen = true }: OmniRightInspectorProps) {
  ...
  if (!open) {
    return (
      ...
      >
        <svg aria-hidden="true" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="m9 6 6 6-6 6" /></svg>
        Inspector
      </button>
    )
  }
  ...
  return (
    <div aria-label="Inspector panel" role="region" className={`relative ${className}`.trim()}>
      <button
