      ...
      {!latestTrace ? <p className="muted-copy">No coordination trace recorded yet.</p> : (
        <>
          <div className="metric-stack">
            <div className="metric-row"><span>Trace</span><strong>{redactRuntimeDebugText(latestTrace.trace_id)}</strong></div>
            <div className="metric-row"><span>Outcome</span><strong>{redactRuntimeDebugText(latestTrace.final_outcome)}</strong></div>
            ...
            {latestTrace.decisions.map((decision) => (
              <article className="timeline-item" key={decision.decision_id}>
                <div className="timeline-item-header">
                  <strong>{redactRuntimeDebugText(decision.specialist_type)}</strong>
                  <span>{Math.round(decision.confidence * 100)}%</span>
                </div>
                <p>{redactRuntimeDebugText(decision.reasoning) || 'No reasoning captured.'}</p>
                <div className="timeline-item-meta">
                  <span>Status: {redactRuntimeDebugText(decision.status)}</span>
                  <span>Simulation: {decision.simulation_id ? redactRuntimeDebugText(decision.simulation_id) : 'n/a'}</span>
                </div>
              </article>
            ))}
          </div>
          ...
          <ul className="observability-list">
            {recentTraces.slice(0, 5).map((trace) => (
              <li key={trace.trace_id}>
                <span>{redactRuntimeDebugText(trace.trace_id)}</span>
                <em>{redactRuntimeDebugText(trace.final_outcome)}</em>
              </li>
            ))}
          </ul>
        </div>
