      ...
      {items.map((item, index) => (
        <article className="signal-item" key={`${String(item.event_type ?? item.run_id ?? index)}-${index}`}>
          <div className="signal-row">
            <strong>{redactRuntimeDebugText(String(item.event_type ?? item.title ?? item.run_id ?? 'event'))}</strong>
            <span>{redactRuntimeDebugText(String(item.timestamp ?? item.updated_at ?? ''))}</span>
          </div>
          <p>{redactRuntimeDebugText(String(item.reason_code ?? item.summary ?? item.message ?? ''))}</p>
        </article>
      ))}
    </div>
  )
