...
fn mock_chat_is_rejected_without_explicit_non_live_authorization() {
    with_clean_mock_env(&[], || {
        let error = validate_mock_mode_for_environment(true).expect_err("mock must fail closed");
...
fn extract_chat_from_python_output_uses_truthful_runtime_session_id() {
    ...
    let invalid =
        extract_chat_from_python_output(r#"{"response":"hi","runtime_session_id":"bad\nsession"}"#);
