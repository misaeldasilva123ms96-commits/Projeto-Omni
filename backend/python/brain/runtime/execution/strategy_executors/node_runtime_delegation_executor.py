...
class NodeRuntimeDelegationExecutor(StrategyExecutorBase):
    ...
    def execute(
        ...
        if compat_execute is None:
            return self.safe_fallback(request, reason="node_runtime_callback_missing", response_text=request.node_fallback_response)
        ...
        trace = self.build_trace(
            request,
            status="success",
            execution_trace_summary="Node runtime delegation executor used the compatibility runtime path with node bridge available.",
            metadata={"delegation_path": "node_runtime_bridge"},
        )
