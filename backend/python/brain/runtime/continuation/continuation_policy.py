...
class DeterministicContinuationPolicy:
    ...
    def from_env() -> ContinuationPolicy:
        return ContinuationPolicy(
            max_retries_per_step=max(0, int(read_env("OMNI_CONTINUATION_MAX_RETRIES_PER_STEP", "2") or "2")),
            allow_replan=read_env("OMNI_CONTINUATION_ALLOW_REPLAN", "true").lower() != "false",
            allow_auto_pause=read_env("OMNI_CONTINUATION_ALLOW_AUTO_PAUSE", "true").lower() != "false",
            allow_auto_escalate=read_env("OMNI_CONTINUATION_ALLOW_AUTO_ESCALATE", "true").lower() != "false",
            max_replans_per_plan=max(0, int(read_env("OMNI_CONTINUATION_MAX_REPLANS_PER_PLAN", "1") or "1")),
        )
