...
function getQueryEngineCandidates() {
  ...
  const backendRuntimePath = path.join(workspaceRoot, 'backend', 'python', 'brain', 'runtime', 'node_runtime', 'index.js');
  const relocatedQueryEnginePath = path.join(workspaceRoot, 'backend', 'python', 'brain', 'runtime', 'query_engine', 'QueryEngine.ts');

  return [
    esmAdapterPath,
    adapterPath,
    backendRuntimePath,
    relocatedQueryEnginePath,
