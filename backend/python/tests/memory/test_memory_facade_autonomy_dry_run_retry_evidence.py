...
class DryRunRetryEvidenceFacadeTest(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = Path(tempfile.mkdtemp(prefix="omni-retry-evidence-test-"))
    ...
    def _record(self, plan_id: str = "plan-1", session_id: str = "session-1") -> DryRunRetryPlanEvidenceRecord:
        return DryRunRetryPlanEvidenceRecord(
            plan_id=plan_id,
            would_retry=True,
            retry_reason="transient_provider_timeout",
            blocked=False,
            block_reasons=[],
            retry_eligibility_score=0.9,
            risk_level="low",
    ...
    def test_sqlite_opt_in_records_and_lists_evidence(self) -> None:
        ...
        results = facade.list_dry_run_retry_plan_evidence()
        ...
        self.assertEqual(payload["suggested_retry_strategy"], "same_provider_safe_retry")
    ...
    def test_sqlite_list_can_filter_by_safe_session_id(self) -> None:
        ...
        facade.record_dry_run_retry_plan_evidence(self._record("plan-1", "session-1"))
        facade.record_dry_run_retry_plan_evidence(self._record("plan-2", "session-2"))

        results = facade.list_dry_run_retry_plan_evidence(session_id="session-2")
    ...
    def test_sqlite_schema_creates_evidence_table(self) -> None:
        ...
        try:
            adapter.connect()
            self.assertTrue(adapter.table_exists("dry_run_retry_plan_evidence"))
        finally:
            adapter.close()
    ...
    def test_sqlite_adapter_insert_and_list_degrades_corrupt_rows(self) -> None:
        ...
        try:
            adapter.connect()
            adapter.insert_dry_run_retry_plan_evidence(self._record())
            self.assertEqual(len(adapter.list_dry_run_retry_plan_evidence()), 1)
            adapter._execute(
                "UPDATE dry_run_retry_plan_evidence SET block_reasons = ? WHERE plan_id = ?",
                "{not-json",
                "plan-1",
            )
            self.assertEqual(adapter.list_dry_run_retry_plan_evidence(), [])
        finally:
            adapter.close()
    ...
    def test_memory_facade_write_failure_degrades_safely(self) -> None:
        ...
        facade.record_dry_run_retry_plan_evidence(self._record())
        self.assertEqual(facade.list_dry_run_retry_plan_evidence(), [])
